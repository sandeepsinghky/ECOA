...
<script>
	...
	if((window.location.href).indexOf("cseOfficeList/1.htm") >= 0)
		{
			form.action = "${pageContext.request.contextPath}/cseOfficeListIter/1.htm";
		}
		else if((window.location.href).indexOf("cseOfficeList/2.htm") >= 0)
		{
			form.action = "${pageContext.request.contextPath}/cseOfficeListIter/2.htm";
		}	
		else if((window.location.href).indexOf("cseOfficeListIter/1.htm") >= 0)
		{
			form.action = "${pageContext.request.contextPath}/cseOfficeListIter/1.htm";
		}	
		else if((window.location.href).indexOf("cseOfficeListIter/2.htm") >= 0)
		{
			form.action = "${pageContext.request.contextPath}/cseOfficeListIter/2.htm";
		}	
		var iPage = parseInt(form.page.value);
	...
	function goToCSEListDetailPage(id, type)
	{		
		if((window.location.href).indexOf("cseOfficeList/1.htm") >= 0)
		{
			location.href="${pageContext.request.contextPath}/cseOfficeDetail/1.htm?id3pty="+id +"&id3ptyType="+type;						
		}
		else if((window.location.href).indexOf("cseOfficeList/2.htm") >= 0)
		{
			location.href="${pageContext.request.contextPath}/cseOfficeDetail/2.htm?id3pty="+id +"&id3ptyType="+type;						
		}
		else if((window.location.href).indexOf("cseOfficeListIter/1.htm") >= 0)
		{
			location.href="${pageContext.request.contextPath}/cseOfficeDetail/1.htm?id3pty="+id +"&id3ptyType="+type;						
...
<div id="content">
  <div class="gutter"> 
	  <table width="100%" border="0">
			<tr><td colspan="2"><b>North Carolina Child Support Enforcement Agency Addresses </b></td></tr>
		...
		<input type="hidden" name="page" value="${page}"> <br>			
	 	<table width="80%" border="0" cellpadding="0" cellspacing="0">	
			...
			<tr>
				...
				<td width="48%" height="20"><font><b>Child Support Agency</b></font></td>
			...
			<c:when test="${dispCocList.size()>0}">
				<c:forEach var="coc" items="${dispCocList}">
					<tr>
						<td width="6%">
							<div align="center">								
								<a href="#" onclick="javascript:goToCSEListDetailPage('${coc.getThirdPartyId().trim()}','${fipsType}')">
	...
	<p>	<input type="hidden" name=fipsType value="${fipsType}">	</p>
